...
function primal_update_old(xk, zk, λk, network, probdata, γ, t, scc_time, z_loc, y_loc; ρ=50, umin=0.2, δmax=10)
    ...
    q_lo = probdata.Qmin[:, t]
    q_up = probdata.Qmax[:, t]
    h_lo = probdata.Hmin[:, t]
    h_up = probdata.Hmax[:, t]
    η_lo = probdata.ηmin[:, t]
    η_up = probdata.ηmax[:, t]
    α_up = probdata.αmax[:, t]
    α_lo = zeros(nn, 1)
    ...
    @variable(model, ψ⁻[i=1:np])
     ...
     reg = 1e-06
     @NLconstraint(model, [i=1:np], r[i]*q[i]*abs(q[i]+reg)^(nexp[i]-1) + sum(A12[i, j]*h[j] for j ∈ nodes_map[i]) + sum(A10[i, j]*h0[j] for j ∈ sources_map[i]) + η[i] == 0)
...
function primal_update(xk, zk, λk, data, γ, t, scc_time; ρ=50, umin=0.2, δmax=10, resto=false, bi_dir=true)
...
function  auxiliary_update(xk, zk, λk, data, γ, pv_type; δmax=10)
    ...
    if pv_type == "variation"
        ...
        @objective(model, Min, sum(sum(-λk[i, k]*z[i, k] + (γ/2)*(hk[i, k] - z[i, k])^2 for i ∈ collect(1:nn)) for k ∈ collect(1:nt)))
    ...
    elseif pv_type == "variability"
        ...
        # @constraint(model, (1/nn)*sum(sum((z[i, k] - mean(z[i, :]))^2 for i ∈ collect(1:nn)) for k ∈ collect(1:nt)) ≤ δmax^2)
        # @constraint(model, [i=1:nn], sum((z[i, k] - mean(z[i, :]))^2 for k ∈ collect(1:nt)) .≤ δmax^2)
        
        # @objective(model, Min, ones(nn)'*(-λk.*z)*ones(nt) + (γ/2)*((vec(z)'*spdiagm((1+reg).*ones(nn*nt))*vec(z)) + (-2*vec(z)'*vec(hk)) + (vec(hk)'*spdiagm(ones(nn*nt))*vec(hk))))
...
function make_object_data(net_name, network, opt_params, v_loc, y_loc)
